...
//! * The schema buffer -- if the segment has an Arrow record batch,
//!   then the schema buffer optionally contains the bytes of the
//!   serialized Arrow schema of that batch.
//! * The header buffer -- this contains the metaversion (memory version,
//!   then batch version) of the written data, followed by an optional name,
//!   e.g. the name of a dataset.
//! * The metadata buffer -- if the segment has an Arrow record batch,
//!   then this contains the Arrow metadata part of the batch.
//! * The data buffer -- if the segment has an Arrow record batch,
//!   then this contains the Arrow data part of the batch. Otherwise,
//!   this can contain arbitrary data, e.g. datasets.
//!
//! If the segment has an Arrow record batch, then the metadata and
//! data buffers above together form the batch in Arrow format.

// TODO: Make separate (i.e. fifth), fixed-size marker and buffer for metaversion?
